...
public class UserSelectorControlEditor extends AbstractControlEditor<CustomControl>
{
	...
	public UserSelectorControlEditor(final Control control, final int wizardType, final SchemaModel schema)
	{
	...
	public void init()
	{
		...
		groupRestriction = new UserGroupPanel(getString("usersel.restrict.groups"), //$NON-NLS-1$
			getString("groups.select"), getString("groups.selected")); //$NON-NLS-1$//$NON-NLS-2$
	...
	{
		return CurrentLocale.get("com.tle.admin.controls.userselector." + partKey); //$NON-NLS-1$
	}

	/**
	 * Very much copy and pasted from DRMAccessControlTab TODO: refactor into a
	 * common component
	 */
	private final class UserGroupPanel extends JGroup
	{
		...
		public void loadControl(UserSelectorControl control)
		{
			...
			{
				setSelected(true);
				Set<String> groupUuids = control.getRestrictedTo(UserSelectorControl.KEY_RESTRICT_USER_GROUPS);
		...
		public void saveControl(UserSelectorControl control)
		{
			...
			Set<String> restrictedTo = control.getRestrictedTo(UserSelectorControl.KEY_RESTRICT_USER_GROUPS);
			restrictedTo.clear();
			if( isSelected() )
			{
