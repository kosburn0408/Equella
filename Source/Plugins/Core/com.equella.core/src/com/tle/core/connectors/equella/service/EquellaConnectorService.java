...
public class EquellaConnectorService implements ConnectorRepositoryImplementation
{
	...
	@Override
	public List<ConnectorContent> findUsages(Connector connector, String username, final String uuid, int version,
		boolean versionIsLatest, boolean archived, boolean allVersion) throws LmsUserNotFoundException
	{
		...
		for( Item item : foundItems )
		{
			...
			final List<CustomAttachment> attachments = new UnmodifiableAttachments(item).getCustomList("resource");
			for( CustomAttachment attachment : attachments )
			{
				...
				{
					if( allVersion || remoteItemVersion == version || (versionIsLatest && remoteItemVersion == 0) )
					{
		...
		boolean reverseSort) throws LmsUserNotFoundException
	{
		...
		final List<CustomAttachment> resources = attachmentDao.findResourceAttachmentsByQuery(query, !archived,
			sortHql);
		...
		while( resIt.hasNext() )
		{
			...
			final Set<String> privs = aclManager.filterNonGrantedPrivileges(item, Collections.singleton(VIEW_ITEM));
			if( privs.isEmpty() )
			{
				resIt.remove();
			}
		...
		return new SimpleSearchResults<ConnectorContent>(content, content.size(), offset, resources.size());
	}
	...
	public int getUnfilteredAllUsagesCount(Connector connector, String username, String query, boolean archived)
	{
		try
		{
			return findAllUsages(connector, username, query, null, null, archived, 0, 0, ExternalContentSortType.NAME,
				false).getAvailable();
		}
	 ...
	 * @return
	 */
	private ConnectorContent convertAttachment(Item item, CustomAttachment attachment, String name, String owner)
	{
		...
		final ViewItemUrl attachmentUrl = attachmentResourceService
			.getViewableResource(info, viewableItemFactory.createNewViewableItem(item.getItemId()/*
																									* new
																									* ItemId (
																									* targetItemUuid
																									* ,
		...
		content.setCourseUrl(itemUrls.createItemUrl(info, item.getItemId(), ViewItemUrl.FLAG_FULL_URL).getHref());
		...
		if( !Check.isEmpty(targetAttachmentUuid) )
		{
			if( targetAttachmentUuid.equalsIgnoreCase("viewims.jsp") )
			{
				...
				List<ImsAttachment> ims = new UnmodifiableAttachments(
					itemService.getUnsecure(new ItemId(targetItemUuid, targetItemVersion))).getList(AttachmentType.IMS);
	...
	public ConnectorFolder addItemToCourse(Connector connector, String username, String courseId, String sectionId,
		IItem<?> item, SelectedResource selectedResource)
	{
