...
@Singleton
public class ActivationServiceImpl implements ActivationService, ItemIdExtension, UserChangeListener
{
	@Inject
	...
	@Override
	@Transactional
	public List<ActivateRequest> getAllRequestsForDateRange(String activationType, Item item, Date start, Date end)
	{
	...
	@Override
	@Transactional
	public List<ActivateRequest> getAllCurrentAndPendingActivations(String activationType, String attachmentUuid)
	{
	...
	public List<ActivateRequest> activate(String activationType, Item item, List<ActivateRequest> requests,
	...
	@Override
	@Transactional
	public void activateAll(String activationType, Map<Long, List<ActivateRequest>> requestMap, boolean ignorePercenage)
	{
	...
	public UtcDate[] getDefaultCourseDates(CourseInfo course)
	{
		...
		if( useGlobalStart )
		{
			CourseDefaultsSettings persistedSettings = systemConstantsService
				.getProperties(new CourseDefaultsSettings());
	...
	public String getStatusKey(int status)
	{
		switch( status )
		{
			case ActivateRequest.TYPE_ACTIVE:
				return "com.tle.core.activation.status.active"; //$NON-NLS-1$
			case ActivateRequest.TYPE_PENDING:
				return "com.tle.core.activation.status.pending"; //$NON-NLS-1$
			case ActivateRequest.TYPE_INACTIVE:
				return "com.tle.core.activation.status.inactive"; //$NON-NLS-1$
		}
		return null;
	}
	...
	{
		doOperationWithReindex(new ItemIdKey(request.getItem()), operationFactory.createDeactivate(request.getId()));
	}
	...
	{
		doOperationWithReindex(new ItemIdKey(request.getItem()), operationFactory.createDelete(request.getId()));
	}
	...
	{
		implTracker = new PluginTracker<ActivationImplementation>(pluginService, "com.tle.core.activation",
			"activationImplementation", "type");
	...
	@Override
	public void validateItem(String activationType, Item item, boolean ignoreOverrides, boolean skipPercentage)
	{
	...
	public void userIdChangedEvent(UserIdChangedEvent event)
	{
		...
		ReassignActivationOperation op = operationFactory.reassignActivations(event.getFromUserId(),
			event.getToUserId());
	...
	@Override
	public boolean attachmentIsSelectableForCourse(String activationType, String attachmentUuid, String courseCode)
	{
		final CourseDefaultsSettings courseSettings = systemConstantsService
			.getProperties(new CourseDefaultsSettings());
		if( courseSettings.isPortionRestrictionsEnabled() )
		{
			final List<ActivateRequest> requests = requestDao.getAllActiveAndPendingRequests(activationType,
				attachmentUuid);
			return requests.stream().filter(r -> r.getCourse().getCode().equals(courseCode)).findFirst().isPresent();
		}
		return true;
	}
}
