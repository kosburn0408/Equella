...
public class ScriptingServiceImpl implements ScriptingService
{
	...
	@Override
	public boolean evaluateScript(final String script, final String scriptName, final ScriptContext context)
		throws ScriptException
	{
		return evaluateInternal(script, scriptName, context);
	}

	private boolean evaluateInternal(final String script, final String scriptName, final ScriptContext context)
		throws ScriptException
	{
	...
	@Override
	public Object executeScript(final String script, final String scriptName, final ScriptContext context,
		final boolean function) throws ScriptException
	{
	...
	public static final class ScriptErrorReporter implements ErrorReporter
	{
		...
		{
			String alteredLine = Utils.safeSubstring(line, 0, linePos - 1) + "<<<"
				+ Utils.safeSubstring(line, linePos - 1);
			return msg + "\n" + CurrentLocale.get("com.tle.core.scripting.error.line", lineNum, alteredLine, linePos);
		}
		...
		public EvaluatorException runtimeError(String msg, String scriptName, int lineNum, String line, int linePos)
		{
			...
			summary.append(CurrentLocale.get("com.tle.core.scripting.error.preamble", scriptName) + "\n");
			summary.append(msg + "\n");

			for( String error : errors )
			{
				summary.append(CurrentLocale.get("com.tle.core.scripting.label.error") + ": " + error + "\n");
			}
			for( String warning : warnings )
			{
				summary.append(CurrentLocale.get("com.tle.core.scripting.label.warning") + ": " + warning + "\n");
			}
